...
and random 16 bit numbers in elements 1 to 8. These random numbers are 
moved to elements 2 to 9 and a new carry and number are generated and 
placed in elements 0 and 1. The arrays mother1 and mother2 are filled with 
random 16 bit values on first call of Mother by another generator. mStart 
is the switch.

Returns:
A 32 bit random number is obtained by combining the output of the two 
generators and returned in *pSeed.  It is also scaled by 2^32-1 and 
returned as a double between 0 and 1
...
double Mother(unsigned long *pSeed)
{
	...
	if (mStart) 
	{
		sNumber= *pSeed&m16Mask;   				/* The low 16 bits */
	  	...
	  	for (n=18;n--;) 
		{
	    	number=30903*sNumber+(number>>16);	/* One line mult-with-cary */
