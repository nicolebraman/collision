...
float p[MAXBALLS][3], v[MAXBALLS][3];
float po[MAXBALLS][3], ve[MAXBALLS][3];
int balArr[MAXCOLL][2];
float timArr[MAXCOLL];
int fBalArr[MAXCOLL][2];
float fTimArr[MAXCOLL];
...
float findCollision(int i, int j) {
	float dx, dy, dz, dvx, dvy, dvz, a, b, c, D, sD, t1, t2, tCol;
...
void processCollision(int b1, int b2, float tc) {
	float dx, dy, dz, vb1d, vb2d, nv;
...
int main(int argc, char * argv[]) {
	printf("hi");
	float tij, maxNegTime, minPosTime;
		...
		*/
		...
		while (more) {
			...
			/* Check all potential collisions */
					...
					printf("\nChecking all potential collisions. Count: %i\n", counter);
					counter++;
